...
const create = async (req, res) => {
  const { activationDate, expirationDate, activated, name, price, features, accessLevel } = req.body;

  try {
    if (!activationDate || !expirationDate || !activated || !name || !price || !features || !accessLevel) {
...
const update = async (req, res) => {
  try {
    const { id } = req.params;
    const {...newSubscriptionData } = req.body; // Exclude createdAt from newPostData
